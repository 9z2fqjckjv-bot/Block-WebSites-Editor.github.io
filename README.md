...
| 🌐 Webサイト読込 | 指定URLのHTMLを読み込み、ブロックへ変換して編集 |
...
| 🎞 マルチメディア埋め込み | 音声・動画・ドキュメント・表計算・プレゼン・PDF・テキスト(txt/md)をブロックから表示 |
| 🧪 Google Apps Script埋め込み | Webアプリとして公開したGoogle Apps Scriptをiframeで配置可能 |
...
4. 既存サイトを再編集したい場合は「🌐 サイト読込」でURLから取り込み
5. 別日に「📂 読込」で続きから再開
6. 完成後にHTMLをダウンロードして公開
...
### ⚙️ スクリプト
`<head> 埋め込みコード` / `<body> 先頭埋め込みコード`（Google Tag / noscript などを配置）

### 📝 テキスト
`見出し (H1–H6)` / `段落` / `テキスト(生)`

### 🖼 メディア
`画像`（表示/非表示・クリックリンク・揃え・配置・重なり順対応） / `音声` / `動画` / `ドキュメント`（Word・Googleドキュメント・Pages） / `表計算シート`（Excel・Googleスプレッドシート・Numbers） / `プレゼンテーション`（PowerPoint・Googleスライド・Keynote） / `PDF` / `テキストファイル`（txt・md） / `Google Apps Script埋め込み` / `区切り線` / `カスタムHTML埋め込み`
